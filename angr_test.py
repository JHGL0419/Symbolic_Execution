...
reg_dump("./entry_point.DMP", entry_state)
# list_1 = parse_regs("./regs.txt", entry_state)      # register는 minidump.py에 아직 구현 안된듯.
list_2 = parse_mem_text("./mem.txt", entry_state)        # stack인데 밑에 처럼 바꾸는게 나을 듯.

# memory load. segment 별로 해줘야 함.
# main_object
parse_dump("./entry_point.DMP", 0x140010000, 0x1000, entry_state)  # .data
parse_dump("./entry_point.DMP", 0x140011000, 0x2000, entry_state)  # .rdata
parse_dump("./entry_point.DMP", 0x140015000, 0x2000, entry_state)  # .bss
...
dll 들어갈 공간...
"""
dll_memory("./entry_point.DMP", "ntdll.dll", entry_state)
